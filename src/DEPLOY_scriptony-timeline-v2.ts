 ...
 * 
 * 🔧 FIX: Characters aus Worldbuilding werden jetzt auch in Timeline angezeigt!
 */
 ...
 * 
 * 🔧 FIX: Findet jetzt auch Worldbuilding-Characters über world_id!
 */
app.get("/timeline-characters", async (c) => {
  try {
    ...
    // First, get the project to find its world_id (for Worldbuilding integration)
    ...
    console.log(`[timeline-characters] Project data:`, { projectId, world_id: project?.world_id, organization_id: project?.organization_id });

    // Now fetch characters
    // Try BOTH:
    // 1. Timeline characters (with project_id)
    // 2. Worldbuilding characters (with world_id + organization_id matching the project)
    let query = supabase
      .from("characters")
      .select("*")
      .order("name", { ascending: true });

    // Build OR condition dynamically based on what the project has
    if (project?.world_id && project?.organization_id) {
      // Project has a world - include both Timeline AND Worldbuilding characters
      query = query.or(`project_id.eq.${projectId},and(world_id.eq.${project.world_id},organization_id.eq.${project.organization_id})`);
    } else if (project?.organization_id) {
      // No world, but has organization - include Timeline characters OR org-wide characters
      query = query.or(`project_id.eq.${projectId},organization_id.eq.${project.organization_id}`);
    } else {
      // Fallback: Just use project_id
      query = query.eq("project_id", projectId);
    }
