...
export function EdgeFunctionDebugPanel() {
  ...
  const testFunction = async (name: string): Promise<FunctionStatus> => {
    ...
    try {
      ...
      if (error.name === 'AbortError' || error.message.includes('timeout')) {
        errorMessage = 'Request timeout (>10s) - function might be cold starting or not responding';
      } else if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
        errorMessage = 'Network error - CORS issue, function not deployed, or network blocked';
      }
  ...
  const getStatusIcon = (status: FunctionStatus['status']) => {
    switch (status) {
      ...
      default:
        return <div className="size-4 rounded-full border-2 border-gray-300 animate-pulse" />;
    }
  };
  ...
  const getStatusBadge = (status: FunctionStatus['status']) => {
    switch (status) {
      ...
      case 'not-deployed':
        return <Badge className="bg-orange-100 text-orange-800 border-orange-200">Not Deployed</Badge>;
      default:
        return <Badge className="bg-gray-100 text-gray-800 border-gray-200">Testing...</Badge>;
    }
  };
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 text-sm">
              ...
              <Badge className="bg-orange-100 text-orange-800 border-orange-200">
                ! {results.filter(r => r.status === 'not-deployed').length} Not Deployed
              </Badge>
            </div>
            ...
            <div className="space-y-2 mt-4">
              {results.map((result) => {
                ...
                return (
                  <Collapsible key={result.name} open={isExpanded} onOpenChange={() => toggleExpanded(result.name)}>
                    <div className="border rounded-lg">
                      <CollapsibleTrigger asChild>
                        <button className="w-full p-3 flex items-center justify-between hover:bg-muted/50 transition-colors">
                          <div className="flex items-center gap-3">
                            {getStatusIcon(result.status)}
                            <code className="text-sm font-mono">{result.name}</code>
                            {result.responseTime && (
                      ...
                      <CollapsibleContent>
                        <div className="border-t p-3 bg-muted/30 text-sm">
                          {result.status === 'success' && result.response && (
                            <div className="space-y-2">
                              <div className="font-medium text-green-700">✓ Function is responding correctly</div>
                              <div className="text-xs">
                                <div className="font-medium text-muted-foreground mb-1">Health Response:</div>
                          ...
                          {result.status === 'error' && (
                            <div className="space-y-2">
                              <div className="font-medium text-red-700">✗ Function Error</div>
                              ...
                              <div className="text-xs text-muted-foreground mt-2">
                                ...
                                <ul className="list-disc list-inside mt-1 space-y-1">
                                  <li>Function not deployed in Supabase Dashboard</li>
                          ...
                          {result.status === 'not-deployed' && (
                            <div className="space-y-2">
                              <div className="font-medium text-orange-700">! Function Not Deployed</div>
                              <div className="text-xs text-muted-foreground">
                                This function needs to be deployed in the Supabase Dashboard.
                              </div>
                            </div>
                          )}
                        </div>
