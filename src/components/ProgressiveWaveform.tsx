...
export function ProgressiveWaveform({
  ...
  const [dragState, setDragState] = useState<DragState>({ type: null, startX: 0, startValue: 0 });
  ...
  useEffect(() => {
    if (!initialPeaks) {
      workerRef.current = new Worker(new URL('/workers/audio-decoder.worker.ts', import.meta.url), {
        type: 'module',
      });
  ...
  useEffect(() => {
    ...
    visiblePeaks.forEach((peak, index) => {
      ...
      const timeInAudio = visibleStartTime + (index / visiblePeaks.length) * visibleDuration;
      let opacity = 1;
  ...
  }, [peaks, duration, width, height, startTime, actualEndTime, fadeIn, fadeOut]);

  // Helper to draw handles
  const drawHandle = (ctx: CanvasRenderingContext2D, x: number, height: number, color: string, label: string) => {
    ctx.fillStyle = color;
  ...
  const drawFadeHandle = (ctx: CanvasRenderingContext2D, x: number, height: number, color: string, label: string) => {
    ctx.fillStyle = color;
    ctx.beginPath();
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (Math.abs(x) < tolerance) {
      ...
      const fadeOutX = width - (fadeOut / (actualEndTime - startTime)) * width;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (dragState.type === 'start') {
      const newStart = Math.max(0, Math.min(actualEndTime - 0.1, dragState.startValue + deltaTime));
      onStartTimeChange?.(newStart);
    } else if (dragState.type === 'end') {
      const newEnd = Math.max(startTime + 0.1, Math.min(duration, dragState.startValue + deltaTime));
      onEndTimeChange?.(newEnd);
    } else if (dragState.type === 'fadeIn') {
      const newFade = Math.max(0, Math.min(actualEndTime - startTime, dragState.startValue + deltaTime));
      onFadeInChange?.(newFade);
    } else if (dragState.type === 'fadeOut') {
      const newFade = Math.max(0, Math.min(actualEndTime - startTime, dragState.startValue - deltaTime));
      onFadeOutChange?.(newFade);
    }
  }, [dragState, width, startTime, actualEndTime, duration, onStartTimeChange, onEndTimeChange, onFadeInChange, onFadeOutChange]);
