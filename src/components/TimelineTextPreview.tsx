...
export function TimelineTextPreview({ 
  ...
  for (let i = 0; i < sentences.length; i++) {
    const sentenceWords = sentences[i].trim().split(/\s+/).filter(w => w.length > 0).length;
  ...
  const displayWords = displayText.trim().split(/\s+/).filter(w => w.length > 0);
  
  // Calculate word offset (how many words before display text)
  let wordsBeforeDisplay = 0;
  for (let i = 0; i < startSentence; i++) {
    wordsBeforeDisplay += sentences[i].trim().split(/\s+/).filter(w => w.length > 0).length;
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-6 px-6">
      ...
      <div className="text-sm text-muted-foreground">
        ...
        {currentSceneTitle && (
          <span className="ml-2 text-muted-foreground/70">• {currentSceneTitle}</span>
        )}
      </div>
    </div>
  );
