    ...
    const autoGenerateBeats = async () => {
      try {
        ...
        if (!beatsData || beatsData.length === 0) {
          if (!beatTemplate) {
            console.log('[StructureBeatsSection] ℹ️ No beats and no template specified');
            return;
          }
          ...
          console.log(`[StructureBeatsSection] ✅ Successfully created ${generatedBeats.length} beats`);
          toast.success(`${generatedBeats.length} Story Beats automatisch generiert`);
        } else {
