...
export function ProjectStatsLogsDialog({ 
  ...
  const loadLogs = async () => {
    try {
      ...
      const response = await fetch(
        `${supabaseConfig.url}/functions/v1/scriptony-logs/logs/project/${project.id}/recent`,
        {
          headers: { 'Authorization': `Bearer ${token}` },
        }
      );
      ...
      setLogsError('Fehler beim Laden der Activity Logs');
    } finally {
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      ...
      case 'updated': return <Edit className="size-4 text-blue-600" />;
      case 'deleted': return <Trash2 className="size-4 text-red-600" />;
              ...
              <>
                ...
                <Card>
                  ...
                  <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="flex flex-col gap-1 p-3 bg-purple-50 dark:bg-purple-950/20 rounded-lg border border-purple-100 dark:border-purple-900">
