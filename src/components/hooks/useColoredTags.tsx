...
export function useColoredTags({ characters, assets, scenes }: UseColoredTagsProps) {
  ...
  const colorizeText = (text: string) => {
    ...
    const parts: Array<{ text: string; type: 'character' | 'asset' | 'scene' | 'normal' }> = [];
    let lastIndex = 0;

    // First, find all exact matches with known names
    const exactMatches: Array<{ start: number; end: number; type: 'character' | 'asset' | 'scene'; text: string }> = [];

    // Check for character matches
    characters.forEach(character => {
      const pattern = new RegExp(`@${character.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?=\\s|$)`, 'g');
      let match;
    ...
    assets.forEach(asset => {
      const pattern = new RegExp(`/${asset.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?=\\s|$)`, 'g');
      let match;
      while ((match = pattern.exec(text)) !== null) {
        exactMatches.push({
          start: match.index,
    ...
    scenes.forEach(scene => {
      const pattern = new RegExp(`#${scene.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?=\\s|$)`, 'g');
      let match;
      while ((match = pattern.exec(text)) !== null) {
        exactMatches.push({
          start: match.index,
    ...
    filteredMatches.forEach(match => {
      ...
      if (match.start > lastIndex) {
        ...
        const partialParts = beforeText.split(/(@[^\s@/#]+|\/[^\s@/#]+|#[^\s@/#]+)/g);
    ...
    if (lastIndex < text.length) {
      ...
      const partialParts = remainingText.split(/(@[^\s@/#]+|\/[^\s@/#]+|#[^\s@/#]+)/g);
      partialParts.forEach(part => {
        if (part) {
          if (part.startsWith('@')) {
            parts.push({ text: part, type: 'character' });
