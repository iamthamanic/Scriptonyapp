...
export function useTimelineCache(projectId: string) {
   ...
   */
  const prefetchTimeline = useCallback((element: HTMLElement | null) => {
    if (!element) return () => {};
   ...
   */
  const prefetchCharacters = useCallback((element: HTMLElement | null) => {
    if (!element) return () => {};
   ...
   */
  const prefetchBeats = useCallback((element: HTMLElement | null) => {
    if (!element) return () => {};
